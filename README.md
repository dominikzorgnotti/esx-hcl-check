...
By default (`GOVC_INSECURE` unset or `0`, and `-k` not passed) the vCenter certificate is verified against your host's **system root CA store**. Enabling insecure mode (`GOVC_INSECURE=1` or `-k`) disables verification entirely — the connection then trusts any certificate and is vulnerable to man-in-the-middle interception, so use it only for trusted, self-signed lab environments.

For everything short of disabling verification, the same `govc` TLS settings are honored. Each has an environment variable and, where `govc` provides one, an equivalent command-line flag; **the flag overrides the environment variable**:

| Variable | Flag | Purpose |
| ----- | ----- | ----- |
| `GOVC_TLS_CA_CERTS` | `-tls-ca-certs` | Path(s) to PEM CA bundle(s) to trust **instead of** the system roots. Separate multiple paths with the OS path-list separator (`:` on Linux/macOS, `;` on Windows). |
| `GOVC_TLS_KNOWN_HOSTS` | `-tls-known-hosts` | Path to a `known_hosts`-style file of `host thumbprint` entries, used as a fallback when normal chain verification fails (thumbprint pinning). |
| `GOVC_TLS_HANDSHAKE_TIMEOUT` | *(none)* | Go duration string (e.g. `10s`) bounding the TLS handshake. |

A missing/invalid CA bundle or an unparseable timeout is a hard error rather than a silent fallback to weaker verification. When insecure mode is on, the CA-bundle and known-hosts settings have no effect (verification is off) and the tool warns you.
...
| `-u` | vCenter/ESX URL. Overrides the `GOVC_URL` variable. | `$GOVC_URL` |
| `-k` | Skip verification of the server certificate (insecure). Overrides the `GOVC_INSECURE` variable. | `$GOVC_INSECURE` |
| `-tls-ca-certs` | PEM CA bundle(s) to trust instead of system roots. Overrides the `GOVC_TLS_CA_CERTS` variable. | `$GOVC_TLS_CA_CERTS` |
| `-tls-known-hosts` | TLS known-hosts thumbprint file. Overrides the `GOVC_TLS_KNOWN_HOSTS` variable. | `$GOVC_TLS_KNOWN_HOSTS` |
...
| `-map` | Lists each network card and storage adapter individually with its ESX device name (`vmnicN` / `vmhbaN`) and link state (`UP`/`DOWN`). See [Mapping adapters to ESX device names](#-mapping-adapters-to-esx-device-names). Shapes `-json`/`-csv` output; defaults to `-json` if neither is given; incompatible with `-unique`. | `false` |
...
* `link_state` — `UP` or `DOWN`. For NICs it reflects the physical link. For storage adapters it is reported only where a link concept applies — fabric/target-attached HBAs (FC/iSCSI) that report `online`/`offline`; **internal RAID/SAS controllers have no link, so `link_state` is omitted for them** rather than showing a misleading `DOWN`.
...
`-map` only affects the machine-readable formats. If you pass `-map` without `-json` or `-csv`, it defaults to `-json` (like `-details`) so the fields are actually emitted. It is the opposite of `-unique` (which aggregates across hosts), so the two cannot be combined.
...
1. **The vSAN Offline Database (local, authoritative for firmware/driver).** Storage controllers and vSAN SSD/NVMe devices are first matched here — by exact hex identifiers (VID, DID, SVID, SSID) for controllers/NVMe, or by vendor + model tokens for disks. A match yields not just a certified baseline but the **arrays of certified driver and firmware versions**, which the tool cross-references against what is actually installed. This is what powers the `driver_certified` / `firmware_certified` columns, the `supported_drivers` / `supported_firmwares` lists (`-details`), and the `-mismatch` filter. Driver versions are matched by prefix so vendor build suffixes don't cause false mismatches.

2. **The live Broadcom Compatibility Guide API (fallback, baseline only).** Anything not covered by the offline DB — the system chassis, the CPU, and **all NICs and other I/O devices** — is checked against the live API. Each device class is matched against its own guide: I/O devices by PCI ID (`program=io`), the system and CPU by model/CPUID, and **GPUs against the dedicated vSphere third-party GPU guide** (`program=sptg`). The GPU guide is fetched once and each GPU is matched by exact **VID:DID** — the reliable join key, since vSphere device names carry form-factor noise (e.g. `NVIDIA H100 NVL PCIe 94GB` vs the guide's `NVIDIA H100 NVL`) and short model tokens collide across siblings (`L4` vs `L40S`). This confirms whether the hardware **baseline** is certified (`TRUE`/`FALSE`) but cannot certify an exact firmware/driver combination, so those columns report `N/A`. Live calls are protected by timeouts and bounded retry/backoff, and identical hardware seen across many hosts is queried **once** and reused (cross-host de-duplication) to avoid hammering the endpoint.
...
> **Out of scope — vSAN RDMA NIC HCL.** The offline database's NIC category is Broadcom's **vSAN RDMA/RoCE NIC** list, which is separate from — and more restrictive than — the general I/O device HCL. Matching general NICs against it produced false negatives (an adapter certified for the current release appearing as supported only up to an old one). The tool therefore **does not** use the vSAN RDMA NIC list: NIC compatibility is verified against the general **I/O device HCL** via the live API. Consequences: NIC verdicts require internet (under `-offline` a NIC is reported `SKIPPED`), and this tool does **not** assess a NIC's suitability for vSAN RDMA/RoCE.
